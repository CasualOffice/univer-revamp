import "../chunk-3XBZGOHB.js";
import {
  UniverDebuggerPlugin
} from "../chunk-4KZWGC4Q.js";
import {
...
var SlideRenderController = class extends RxDisposable {
  ...
  _createSlide(mainScene) {
    ...
    if (!model) {
      return new Slide("__slideRender__" /* COMPONENT */, { left: 0, top: 0, width: 0, height: 0, zIndex: 10 });
    }
  ...
  createPageScene(pageId, page) {
    ...
    const snapshot = (_a = this._renderContext.unit) == null ? void 0 : _a.getSnapshot();
    const pageElements = resolvePlaceholders(page, snapshot != null ? snapshot : {});
    const colorScheme = (_b = snapshot == null ? void 0 : snapshot.theme) == null ? void 0 : _b.colorScheme;
...
// ../packages/slides-ui/src/commands/mutations/element.mutation.ts
var SlideInsertElementMutation = {
  id: "slide.mutation.insert-element",
  type: 2 /* MUTATION */,
  handler: (accessor, params) => {
    if (!params) return false;
    const instances = accessor.get(IUniverInstanceService);
    const model = instances.getUnit(params.unitId);
...
var SlideDeleteElementMutation = {
...
var SlideUpdateElementMutation = {
  ...
  handler: (accessor, params) => {
    ...
    page.pageElements[params.elementId] = merge_default(existing, params.props);
    model.updatePage(params.pageId, page);
    model.incrementRev();
    return true;
  }
};
var SlideInsertPageMutation = {
...
var SlideUpdatePageMutation = {
  ...
  handler: (accessor, params) => {
    ...
    const page = model.getPage(params.pageId);
    if (!page) return false;
    const merged = merge_default(page, params.patch);
    model.updatePage(params.pageId, merged);
    model.incrementRev();
    return true;
  }
};
var SlideDeletePageMutation = {
...
var AppendSlideOperation = {
  ...
  handler: (accessor, params) => {
    var _a, _b;
    if (!(params == null ? void 0 : params.unitId)) return false;
    ...
    const pageOrder = (_a = model.getPageOrder()) != null ? _a : [];
    const activeId = (_b = model.getActivePage()) == null ? void 0 : _b.id;
    const activeIdx = activeId ? pageOrder.indexOf(activeId) : -1;
    const insertIndex = activeIdx >= 0 ? activeIdx + 1 : void 0;
    const insertParams = {
...
// ../packages/slides-ui/src/commands/operations/clipboard.operation.ts
var clipboardBuffer = [];
var PASTE_OFFSET = 10;
...
function pasteElements(accessor, unitId, pageId, source) {
  var _a, _b;
  ...
  for (const buffered of source) {
    ...
    element.left = ((_a = element.left) != null ? _a : 0) + PASTE_OFFSET;
    element.top = ((_b = element.top) != null ? _b : 0) + PASTE_OFFSET;
    const insertParams = {
      unitId,
      pageId,
      element
...
var SlideDuplicateElementCommand = {
...
var DeleteSlideElementOperation = {
  ...
  type: 0 /* COMMAND */,
  handler: (accessor, params) => {
    if (!(params == null ? void 0 : params.id) || !params.unitId) return false;
    const commandService = accessor.get(ICommandService);
    const undoRedoService = accessor.get(IUndoRedoService);
    const instances = accessor.get(IUniverInstanceService);
    const model = instances.getUnit(params.unitId);
...
var InsertSlideFloatImageCommand = {
  ...
  handler: async (accessor) => {
    ...
    const undoRedoService = accessor.get(IUndoRedoService);
    const instances = accessor.get(IUniverInstanceService);
    const unitId = (_a = instances.getCurrentUnitOfType(3 /* UNIVER_SLIDE */)) == null ? void 0 : _a.getUnitId();
...
function insertShape(accessor, unitId, buildElement) {
  if (!unitId) return false;
  const commandService = accessor.get(ICommandService);
  const undoRedoService = accessor.get(IUndoRedoService);
  const instances = accessor.get(IUniverInstanceService);
...
var InsertSlideShapeRectangleCommand = {
  ...
  handler: (accessor) => {
    var _a;
    const unitId = (_a = accessor.get(IUniverInstanceService).getFocusedUnit()) == null ? void 0 : _a.getUnitId();
    const commandService = accessor.get(ICommandService);
...
var InsertSlideShapeRectangleOperation = {
  ...
  type: 0 /* COMMAND */,
  handler: (accessor, params) => insertShape(accessor, params == null ? void 0 : params.unitId, (id, zIndex) => ({
    id,
    zIndex,
    left: 378,
    top: 142,
...
var SlideAddTextOperation = {
  ...
  handler: (accessor, params) => {
    ...
    const maxIndex = elements.length ? Math.max(...elements.map((element2) => element2.zIndex)) : 21;
...
// ../packages/slides-ui/src/commands/operations/duplicate-slide.operation.ts
var SlideDuplicateSlideCommand = {
  ...
  handler: (accessor, params) => {
    ...
    const undoRedoService = accessor.get(IUndoRedoService);
    const instances = accessor.get(IUniverInstanceService);
    const unitId = (_b = params == null ? void 0 : params.unitId) != null ? _b : (_a = instances.getFocusedUnit()) == null ? void 0 : _a.getUnitId();
    if (!unitId) return false;
    const model = instances.getUnit(unitId);
    if (!model) return false;
    const sourcePage = (params == null ? void 0 : params.pageId) ? model.getPage(params.pageId) : model.getActivePage();
    ...
    for (const original of Object.values((_c = sourcePage.pageElements) != null ? _c : {})) {
    ...
    const insertParams = {
      unitId,
      page: clone,
      index: sourceIdx === -1 ? void 0 : sourceIdx + 1
    };
    const deleteParams = {
      unitId,
      pageId: newPageId
...
var SlideDeleteSlideCommand = {
  ...
  handler: (accessor, params) => {
    ...
    const model = instances.getUnit(unitId);
    if (!model) return false;
    const page = (params == null ? void 0 : params.pageId) ? model.getPage(params.pageId) : model.getActivePage();
    if (!page) return false;
    const order = model.getPageOrder();
...
var SlideEditingRenderController = class extends Disposable {
   ...
   */
  _getLiveDocSelectionRenderService() {
    const renderer = getCurrentTypeOfRenderer(1 /* UNIVER_DOC */, this._instanceSrv, this._renderManagerService);
  ...
  _initialCursorSync(d) {
    d.add(this._cellEditorManagerService.focus$.pipe(filter((f) => !!f)).subscribe(() => {
      var _a;
      (_a = this._getLiveDocSelectionRenderService()) == null ? void 0 : _a.sync();
    }));
  }
  /**
  ...
  _subscribeToCurrentCell(d) {
    d.add(this._editorBridgeService.currentEditRectState$.subscribe((editCellState) => {
      ...
      (_a = this._getLiveDocSelectionRenderService()) == null ? void 0 : _a.activate(HIDDEN_EDITOR_POSITION2, HIDDEN_EDITOR_POSITION2);
    }));
  }
  /**
